...
%FrameStep – How many frames to skip when reading the track subset. If every frame is to be
%read set this value to 1.
%MaxMissingFrames – This value indicates if tracks not present in the current frame should be
%included in the track subset and if so how many frames away from the current frame a track
%is allowed to be and still be included in the subset. Setting this value to zero ensures that only
%tracks present in the current frame are included.
%OffsetFrame – The number of frames from the current frame the subset should include. This
