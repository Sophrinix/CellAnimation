...
%assayBrightFieldCytoTestWG - This assay has been optimized for tracking cells imaged using bright-field microscopy using 
...
%ancestry information to  spreadsheets. For each tracked image,  an overlayed image is 
...
%variable would be 'c:/sample images/high-density'.  ImageFilesRoot - String variable specifying the root image 
...
%'Experiment-0002_Position(8)_t021.tif' the root image file name will be 'Experiment-0002_Position(8)_t'.  ImageExtension - String variable 
...
%folder where the overlayed images and track data will be  saved. By  
...
%where the overlayed images and ancestry data will be saved. By  default  
...
%point. By default this  is  set to to a file named 'shapes.csv' 
...
%the ImageFilesRoot and should not be changed. GradientThreshold - Number  specifying the threshold 
...
%area for the areaFilterLabel, clearSmallObjects, segmentObjectsUsingClusters filters. Objects below  this value will be 
...
%set to true in the generateBinImgUsingGradient module. ResizeImageScale - Number specifying by  what 
...
%with the other track.  Used  by detectMergeCandidatesUsingDistance module. MaxSearchRadius - Number specifying 
%the absolute lower bound for the search  radius to prevent selecting  too 
%few candidate objects for a track. Used by  assignCellToTrackUsingAll module. MinSearchRadius - Number 
%specifying the absolute higher bound for the search radius  to prevent selecting  
%too many candidate objects for a track. Used by assignCellToTrackUsingAll  module. MinSecondDistance - 
...
%  Used by assignCellToTrackUsingAll module. MaxAngleDiff - Number specifying the maximum allowed angle 
...
%object. Used by assignCellToTrackUsingAll  module. NrParamsForSureMatch - Number specifying the minimum number of 
...
%by assignCellToTrackUsingAll module. SearchRadiusPct - Number specifying  the size of the neighborhood from 
...
%mitotic event.  Used by detectMitoticEvents  module. MinSplitEccentricity - Number specifying the minimum 
%eccentricity a new  nucleus may have and still be  considered as part 
%of a possible mitotic  event. Used by detectMitoticEvents module. MaxSplitEccentricity - Number specifying 
...
%list of objects.  Used by assignCellToTrackUsingAll module. DefaultParamWeights - Numeric array specifying a 
...
% by assignCellToTrackUsingAll  module. DistanceRankingOrder - Numeric array specifying the default order of 
%shape and motility parameters  for slow  moving objects when it cannot be 
%determined based on prediction power.  Used by assignCellToTrackUsingAll  module. DirectionRankingOrder - Numeric 
...
%by  assignCellToTrackUsingAll module. RelevantParametersIndex - Boolean array  specifying column indexes in the 
...
%provided in TracksLayout variable. Used by assignCellToTrackUsingAll module. UnknownParamWeights - Numeric array  specifying 
%a set of weights to be assigned to shape and  motility parameters  
%when the prediction power of the parameters cannot be determined. Used by  assignCellToTrackUsingAll 
...
%in this variable  is used. Used by  assignCellToTrackUsingAll module. Important Modules areaFilterLabel, 
%assignCellToTrackUsingAll, clearSmallObjects, detectMergeCandidatesUsingDistance, detectMitoticEvents,  generateBinImgUsingGradient, removeShortTracks, segmentObjectsUsingClusters, segmentObjectsUsingMarkers, splitTracks.
