...
%String variable that specifies the absolute location of the directory which contains the  
%time-lapse images. An example of such a string variable would be 'c:/sample images/high-density'. ImageFilesRoot 
%- String variable specifying the root image file name. The root image file name 
% for a set of images is the image file name of any of 
%the  images without the number or the file extension. For example, if the 
%file name  is 'Experiment-0002_Position(8)_t021.tif' the root image file name will be 'Experiment-0002_Position(8)_t'. ImageExtension 
%- String variable specifying the image file extension including the preceding dot. For example 
...
%value for this variable depends on the numbering of the image sequence so if 
% the first image in the sequence is 'image003.tif' then the minimum value is 
%3. FrameCount - Number specifying how many images from the image sequence should be 
...
%if the file name is 'image000020.jpg' the value should  be '%06d'. OutputFolder - 
...
% the images to be analyzed are located. TracksFolder - The folder where the 
%label matrixes containing the cell outlines are saved. By  default this value is 
%set to a folder named 'track' within the output folder. SegmentationFilesRoot - The root 
%file name of the label matrixes containing the cell outlines. ImageFileBase - The path 
%name to the images. This value is generated from the ImageFolder  and the 
%ImageFilesRoot and should not be changed. ApproximationDistance - Number specifying how close the convex 
...
%chance of detecting insignificant concavities. BrightnessThresholdPct - Number specifying the percentage threshold value for 
%the image generated by the generateBinImgUsingLocAvg  filter. Any pixel in the original image 
%smaller than the threshold value times the  corresponding value in the local average 
%image below this value will be set to  zero while the rest will 
%be set to one. ClearBorder - Boolean value specifying whether objects next to or 
%touching the image border in  the binary images generated by the generateBinImgUsingGradient module 
%will be erased (true) or not  (false). ClearBorderDist - Number specifying how close 
%to the border objects may be and still be  erased if the ClearBorder 
%parameter is set to true in the generateBinImgUsingGradient module. MedianFilterSize - Number specifying the 
%size of the median filter used by the distanceWatershed module.  Setting this to 
%a higher integer value will reduce the number of objects detected  by the 
%module and can be used to prevent oversegmentation. ObjectArea - Number specifying the threshold 
%area for the clearSmallObjects, polygonalAssistedWatershed filter. Objects below this  value will be removed 
...
%the generateBinImgUsingLocAvg module. ResizeImageScale - Number specifying by what ratio the images will be 
%resized before they are  processed. By default this value is set to 0.5. 
%Used by the resizeImage module. Important Modules - clearSmallObjects, distanceWatershed, generateBinImgUsingLocAvg, getConvexObjects, manualSegmentationReview, polygonalAssistedWatershed, 
