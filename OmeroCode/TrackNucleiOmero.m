function []=TrackNucleiOmero(dataset_number, image_id)
...
%minimum id in dataset, used for indexing
%global minId;
...
dataset_number
dataIds = java.util.ArrayList;
dataIds.add(java.lang.Long(dataset_number));
dataIds
datasets = g.getDatasets(dataIds, true);
datasets
ds1 = datasets.get(0);
dataset_name = char(ds1.getName().getValue());

image = getImage(g, image_id);
well = char(image.getName().getValue());

well_name = [dataset_name 'Well' well];

%name_idx=well_folder%find(well_folder==ds,2,'last');
%generate a unique well name
%well_name=well_folder((name_idx(1)+1):end);
%well_name(name_idx(2)-name_idx(1))=[];
%well_name(well_name==' ')=[];
TrackStruct.OutputFolder=[root_folder ds 'output' ds well_name '-' watersed_med];
...
mkdir(xls_folder);
TrackStruct.ProlXlsFile=[xls_folder ds well_name '-' watershed_med '.csv'];
TrackStruct.ShapesXlsFile=[xls_folder ds well_name '-' watershed_med '_shapes.csv'];
...
%finds the first id of the first image in the image stack
%set_minId_function.InstanceName='SetMinimumImageId';
%set_minId_function.FunctionHandle=@findMinId;
%set_minId_function.FunctionArgs.DatasetIdNum.Value=dataset_number;

%finds the correct image id based on the loop iteration
make_img_id_function.InstanceName='MakeImageIdsInSegmentationLoop';
make_img_id_function.FunctionHandle=@makeImgId;
make_img_id_function.FunctionArgs.CurFrame.FunctionInstance='SegmentationLoop';
make_img_id_function.FunctionArgs.CurFrame.OutputArg='LoopCounter';
...
image_read_loop.LoopFunctions=[{display_curtrackframe_function};{make_img_id_function};{read_image_function};...
...
%find correct image id in image stack based on overlay loop iteration
make_img_id_function2.InstanceName='MakeImageIdsInOverlayLoop';
make_img_id_function2.FunctionHandle=@makeImgId;
make_img_id_function2.FunctionArgs.CurFrame.FunctionInstance='ImageOverlayLoop';
make_img_id_function2.FunctionArgs.CurFrame.OutputArg='LoopCounter';

%load correct image from Omero based on overlay loop iteration
read_image_function2.InstanceName='ReadImagesInOverlayLoop';
read_image_function2.FunctionHandle=@readImageOmero;
read_image_function2.FunctionArgs.ImageId.FunctionInstance='MakeImageIdsInOverlayLoop';
read_image_function2.FunctionArgs.CurFrame.FunctionInstance='ImageOverlayLoop';
read_image_function2.FunctionArgs.CurFrame.OutputArg='LoopCounter';
read_image_function2.FunctionArgs.ImageId.Value=image_id;
...
image_overlay_loop.LoopFunctions=[{make_img_id_function2};{read_image_function2};{get_cur_tracks2_function};{make_mat_name_function};...
...
functions_list=[{display_trackstruct_function};{set_minId_function};{image_read_loop};{save_tracks_function};{save_matching_groups_function};...
